#!/usr/bin/env bash
# Retention for Authentik PostgreSQL backups — same shape as
# prune-stateful-lab-backups.sh, own backup root so the two backup
# families are never mixed by a path mistake.
#
# Usage: scripts/prune-authentik-backups.sh [--dry-run|--apply]
...
# --- path safety: this is the most destructive script in this milestone ---
...
case "${resolved_root}" in
  */aegis/backups/authentik/postgresql) ;;
  *)
    echo "error: backup root '${resolved_root}' does not look like an authentik postgresql backup directory" >&2
    echo "       expected a path ending in aegis/backups/authentik/postgresql" >&2
    exit 1
    ;;
esac
...
for dir in "${verified_dirs[@]}"; do
  ts="$(basename "${dir}")"
  sidecar="${dir}/authentik-postgresql-${ts}.verified.txt"

#!/usr/bin/env bash
# Creates an encrypted, off-host logical backup of Authentik's own
# PostgreSQL instance on home-k3s. Same proven shape as
# backup-stateful-lab-postgres.sh (lock-free single-shot dump, run from
# the operator's own machine, never from the K3s host itself) — a
# separate script rather than a shared/parameterized one, since this
# project already has two hardcoded, independently-tested backup
# scripts and generalizing both correctly was judged riskier than
# copying the pattern for a second target.
#
# pg_dump runs INSIDE the authentik-postgresql-0 container (kubectl
# exec), matching the exact running PostgreSQL version with no local
# client dependency, over the container's local Unix socket (trusted,
# no password ever touches this script).
#
# The fingerprint is deliberately NOT a full-table hash — Authentik's
# schema is large and has volatile tables (sessions, events, task logs)
# that change on every request. Instead it's an application-level
# invariant: the DB-only test user's username+is_active, and the
# Grafana OAuth application's slug — the exact two facts this
# milestone's persistence proof depends on, and both stable across an
# ordinary restart.
#
...
echo "==> pre-backup identity invariant"
...
pvc_uid="$(kubectl -n "${NAMESPACE}" get pvc data-authentik-postgresql-0 -o jsonpath='{.metadata.uid}' 2>/dev/null || echo unknown)"
pv_name="$(kubectl -n "${NAMESPACE}" get pvc data-authentik-postgresql-0 -o jsonpath='{.spec.volumeName}' 2>/dev/null || echo unknown)"
pv_uid="$(kubectl get pv "${pv_name}" -o jsonpath='{.metadata.uid}' 2>/dev/null || echo unknown)"

artifact="${dest_dir}/authentik-postgresql-${timestamp}.dump.age"
...
metadata="${dest_dir}/authentik-postgresql-${timestamp}.metadata.txt"
cat > "${metadata}" <<EOF
...
pre_backup_identity_fingerprint_sha256: ${fingerprint}
git_revision: ${git_revision}
pvc_name: data-authentik-postgresql-0
pvc_uid: ${pvc_uid}
pv_name: ${pv_name}
pv_uid: ${pv_uid}
